...
interface SentimentResponse {
  ...
  summary: string;
  detailed_analyses: SentimentAnalysis[];
}
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "Sentiment analysis failed");
      }
      ...
      <header className="bg-light-bg py-4 shadow-sm">
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex space-x-6">
            ...
            <Link href="/sentiment" className="text-[#1a1f36] hover:text-gray-800 border-b-2 border-[#1a1f36]">Sentiment</Link>
          </nav>
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="p-4 bg-light-bg rounded-lg">
                    ...
                    <div className={`text-xl font-semibold ${getSentimentColor(results.average_sentiment)}`}>
                      {(results.average_sentiment * 100).toFixed(1)}%
                    </div>
                  </div>
                  <div className="p-4 bg-light-bg rounded-lg">
                    <div className="text-sm text-gray-600 mb-1">Articles Analyzed</div>
                    <div className="text-xl font-semibold text-gray-800">{results.articles_analyzed}</div>
                  </div>
                  <div className="p-4 bg-light-bg rounded-lg">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 mb-8">
              <div className="flex flex-col items-center justify-center h-64">
                ...
                <p className="text-gray-600 text-center max-w-md">
                  Enter a stock ticker and select a time period to analyze recent news sentiment.
                </p>
              </div>
            </div>
          )}
